...
Документация продукта находится:

```text
docs/product-book/
```

Product Book является основным источником требований к архитектуре продукта.
...
- `Таблица_категорий_для_расчёта_вознаграждения_28082026-2_1784189410.xlsx` — комиссии Ozon, действует с `28.08.2026`.
...
- Для Ozon схема DBS использует обычную RFBS-комиссию из колонки `M` файла категорий как ближайший доступный тарифный аналог; специальная колонка `N` для мототехники игнорируется.
- До `28.08.2026` рядом с расчетом комиссии Ozon показывается справочная пометка о будущей таблице; с даты вступления она пропадает автоматически.
